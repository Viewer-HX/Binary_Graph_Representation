...
build_log_dir = "../KernelBuildLog"
clang_log_prefix_name = "linuxBuildLogClang"
gcc_log_prefix_name = "linuxBuildLogGCC"
clang_compiler_cmd_dir = "../Kernel_compiler_cmd/clang"
gcc_compiler_cmd_dir = "../Kernel_compiler_cmd/gcc"
...
def main():
    ...
    for log in log_files_list:
        if clang_log_prefix_name in log:
            version = log[log.index(clang_log_prefix_name):].replace(clang_log_prefix_name,"").replace(".txt","")
            cmds_ver_file = clang_compiler_cmd_dir +"/"+ version +".json"
        ...
        if gcc_log_prefix_name in log:
            version = log[log.index(gcc_log_prefix_name):].replace(gcc_log_prefix_name,"").replace(".txt","")
            cmds_ver_file = gcc_compiler_cmd_dir +"/"+ version +".json"
            if os.path.exists(cmds_ver_file):
                continue
